...
const AITextNode = memo(({ data, selected }: NodeProps) => {
  ...
  return (
    <div className={`relative shadow-av-md rounded-lg bg-av-bg-secondary border-2 min-w-[250px] max-w-[300px] ${
      selected ? 'border-violet-500' : 'border-av-border-subtle'
    }`}>
      ...
      <div className="px-3 py-2 bg-violet-500/10 border-b border-av-border-subtle rounded-t-lg flex items-center justify-between">
        <span className="text-xs font-medium text-violet-400">✨ AI 文案</span>
        {data.state === 'running' && <span className="text-xs text-av-state-warning animate-pulse">执行中...</span>}
      ...
      <div className="px-3 py-2 space-y-1">
        {config.style && <p className="text-xs text-av-text-tertiary">风格：{config.style}</p>}
        {config.word_count && <p className="text-xs text-av-text-tertiary">字数：约{config.word_count}字</p>}
        {config.prompt && <p className="text-xs text-av-text-primary line-clamp-2">📝 {config.prompt}</p>}
        {result.text && <p className="text-xs text-av-state-success line-clamp-3 mt-1 bg-av-state-success/10 p-1 rounded">📄 {result.text}</p>}
      </div>
