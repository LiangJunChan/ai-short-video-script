  ...
  return (
    <div className="pt-4 border-t border-av-border-subtle">
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-medium text-av-text-secondary">标签</span>
      </div>

      <div className="flex flex-wrap gap-2 mb-3">
        {videoTags.length === 0 ? (
          <span className="text-sm text-av-text-tertiary">还没有添加标签</span>
        ) : (
      ...
      <div className="relative">
        ...
        {showTagSuggestions && tagSuggestions.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-av-bg-elevated border border-av-border-subtle rounded-xl shadow-av-lg p-3 z-av-dropdown animate-slide-down">
            <div className="flex flex-wrap gap-1.5">
              {tagSuggestions.map((tag: Tag) => (
                <button
                  key={tag.id}
                  onClick={() => onAddTag(tag.name)}
                  className="px-2.5 py-1.5 bg-av-bg-tertiary hover:bg-primary/10 hover:text-primary rounded-lg text-sm transition-colors"
                >
                  #{tag.name}
                </button>
              ))}
